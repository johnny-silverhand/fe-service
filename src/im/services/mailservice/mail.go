...
func SendMailUsingConfig(to, subject, htmlBody string, config *model.Config) *model.AppError {
	...
	return SendMailUsingConfigAdvanced(to, to, fromMail, replyTo, subject, htmlBody, nil, nil, config)
}

// allows for sending an email with attachments and differing MIME/SMTP recipients
func SendMailUsingConfigAdvanced(mimeTo, smtpTo string, from, replyTo mail.Address, subject, htmlBody string, attachments []*model.FileInfo, mimeHeaders map[string]string, config *model.Config) *model.AppError {
	...
	fileBackend, err := filesstore.NewFileBackend(&config.FileSettings)
	if err != nil {
		return err
	}
