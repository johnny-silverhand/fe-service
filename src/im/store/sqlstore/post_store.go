...
func (s SqlPostStore) GetAllMessages(userId string, offset int, limit int, allowFromCache bool, limitMin int64) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		_, err := s.GetReplica().Select(&posts, "SELECT * FROM Posts WHERE ChannelId IN "+
			...
			" AND DeleteAt = 0 AND CreateAt >= :StartTime AND CreateAt <= :EndTime AND (Type = '' OR Type = 'system_close_channel'  OR Type = 'system_metadata' OR Type = 'system_transaction'  OR Type = 'system_close_unresolved_channel' OR Type = 'system_channel_deleted_message' OR Type IS NULL)"+
...
FROM
     Posts
WHERE
    (Type = '' OR Type = 'system_close_channel' OR Type = 'system_metadata' OR Type = 'system_transaction' OR Type = 'system_close_unresolved_channel' OR Type = 'system_channel_deleted_message' OR Type IS NULL) AND
