	...
	return store.Do(func(result *store.StoreResult) {
		...
		if err := s.GetMaster().Insert(post); err != nil {
			...
			if post.Type != model.POST_JOIN_LEAVE && post.Type != model.POST_ADD_REMOVE &&
				...
				s.GetMaster().Exec("UPDATE Channels SET LastPostAt = GREATEST(:LastPostAt, LastPostAt), LastPostMessage =:LastPostMessage, TotalMsgCount = TotalMsgCount + 1 WHERE Id = :ChannelId", map[string]interface{}{"LastPostAt": time, "ChannelId": post.ChannelId, "LastPostMessage": post.Message})
			} else {
				// don't update TotalMsgCount for unimportant messages so that the channel isn't marked as unread
				s.GetMaster().Exec("UPDATE Channels SET LastPostAt = :LastPostAt, LastPostMessage =:LastPostMessage WHERE Id = :ChannelId AND LastPostAt < :LastPostAt", map[string]interface{}{"LastPostAt": time, "ChannelId": post.ChannelId, "LastPostMessage": post.Message})
			}
...
func (s SqlPostStore) GetAllMessages(userId string, offset int, limit int, allowFromCache bool, limitMin int64) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if limit > 1000 {
			result.Err = model.NewAppError("SqlMessageStore.GetAllMessages", "store.sql_post.get_posts.app_error", nil, "userId="+userId, http.StatusBadRequest)
			return
		}
		...
		_, err := s.GetReplica().Select(&posts, "SELECT * FROM Posts WHERE ChannelId IN "+
			...
			" AND DeleteAt = 0 AND CreateAt >= :StartTime AND CreateAt <= :EndTime AND (Type = '' OR Type = 'system_close_channel'  OR Type = 'system_metadata' OR Type = 'system_transaction' OR Type = 'system_invoice' OR Type = 'system_close_unresolved_channel' OR Type = 'system_channel_deleted_message' OR Type IS NULL)"+
		...
		/*		_, err := s.GetReplica().Select(&posts, `SELECT * FROM Posts LEFT JOIN Channels as CC ON CC.Id = Posts.ChannelId AND CC.CreatorId = :UserId
				...
				ORDER BY Posts.CreateAt DESC OFFSET :Offset ROWS FETCH NEXT (:Limit) ROWS ONLY`, map[string]interface{}{"UserId": userId, "Offset": offset, "Limit": limit})*/
...
func (s SqlPostStore) GetAllMessagesSince(userId string, time int64, allowFromCache bool, limitMin int64) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
...
FROM
     Posts
WHERE
    (Type = '' OR Type = 'system_close_channel' OR Type = 'system_metadata' OR Type = 'system_transaction' OR Type = 'system_invoice' OR Type = 'system_close_unresolved_channel' OR Type = 'system_channel_deleted_message' OR Type IS NULL) AND
...
ORDER BY UpdateAt
    LIMIT 1000 OFFSET 0`,
		...
		if err != nil {
			...
			//lastMessageTimeCache.AddWithExpiresInSecs(channelId, latestUpdate, LAST_MESSAGE_TIME_CACHE_SEC)
...
func (s SqlPostStore) GetAllMessagesBefore(userId string, postId string, numMessages int, offset int, limitMin int64) store.StoreChannel {
	return s.getAllMessagesAround(userId, postId, numMessages, offset, true, limitMin)
}

func (s SqlPostStore) GetAllMessagesAfter(userId string, postId string, numMessages int, offset int, limitMin int64) store.StoreChannel {
	return s.getAllMessagesAround(userId, postId, numMessages, offset, false, limitMin)
}

func (s SqlPostStore) getAllMessagesAround(userId string, postId string, numMessages int, offset int, before bool, limitMin int64) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		var direction string
		var sort string
		if before {
	...
	return store.Do(func(result *store.StoreResult) {
		...
		if err := s.GetReplica().SelectOne(&post, "SELECT * FROM Posts WHERE Props = :Props AND (Type = :Type OR Type = :TypeInvoice)", map[string]interface{}{"Props": props, "Type": model.POST_WITH_METADATA, "TypeInvoice": model.POST_WITH_INVOICE}); err != nil {
			if err == sql.ErrNoRows {
				...
				result.Err = model.NewAppError("SqlPostStore.FindPostWithOrder", "store.sql_post.find_post_with_order.existing.app_error", nil, "order_id="+orderId+", "+err.Error(), http.StatusInternalServerError)
			}
		} else {
			result.Data = &post
		}
