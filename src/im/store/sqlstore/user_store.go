...
func (us SqlUserStore) SaveWithCheckExist(user *model.User) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if user.PhoneVerified == true {
			result.Err = model.NewAppError("SqlUserStore.Save", "store.sql_user.save.phone_verified.app_error", nil, "user_id="+user.Id, http.StatusInternalServerError)
		}
...
func (us SqlUserStore) VerifyPhone(userId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if _, err := us.GetMaster().Exec("UPDATE Users SET PhoneVerified = 1 WHERE Id = :UserId", map[string]interface{}{"UserId": userId}); err != nil {
			result.Err = model.NewAppError("SqlUserStore.VerifyPhone", "store.sql_user.verify_phone.app_error", nil, "userId="+userId+", "+err.Error(), http.StatusInternalServerError)
		}
...
func (us SqlUserStore) VerifyPhoneNew(userId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		if _, err := us.GetMaster().Exec("UPDATE Users SET Phone = PhoneNew WHERE Id = :UserId", map[string]interface{}{"UserId": userId}); err != nil {
			result.Err = model.NewAppError("SqlUserStore.VerifyPhoneNew", "store.sql_user.verify_phone.app_error", nil, "userId="+userId+", "+err.Error(), http.StatusInternalServerError)
		}
...
func (us SqlUserStore) GetByPhone(phone string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if err := us.GetReplica().SelectOne(&user, "SELECT * FROM Users WHERE Phone = :Phone", map[string]interface{}{"Phone": phone}); err != nil {
			result.Err = model.NewAppError("SqlUserStore.GetByPhone", store.MISSING_ACCOUNT_ERROR, nil, "phone="+phone+", "+err.Error(), http.StatusInternalServerError)
		}
...
func (us SqlUserStore) GetByPhoneApp(phone string, appId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		if err := us.GetReplica().SelectOne(&user, "SELECT * FROM Users WHERE Phone = :Phone AND AppId = :AppId AND Roles = :Roles AND DeleteAt = :DeleteAt", map[string]interface{}{"Phone": phone, "AppId": appId, "Roles": model.CHANNEL_USER_ROLE_ID, "DeleteAt": 0}); err != nil {
...
func (us SqlUserStore) AccrualBalance(userId string, value float64) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		curTime := model.GetMillis()
		if _, err := us.GetMaster().Exec("UPDATE Users SET Balance = Balance + :Value, UpdateAt = :Time WHERE Id = :UserId", map[string]interface{}{"UserId": userId, "Value": math.Abs(value), "Time": curTime}); err != nil {
