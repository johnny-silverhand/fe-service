...
func (s SqlChannelStore) GetChannelsForUser(userId string, includeDeleted bool, status string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		...
		WHERE CM.UserId = :UserId AND C.Status = :Status AND C.DeleteAt = 0 AND ((C.Id = CWO.ChannelId AND O.DeliveryAt < :DeliveryAt) OR O.Id IS NULL OR C.Status = :Status)
		...
		if includeDeleted {
			...
			LEFT JOIN (SELECT SUBSTRING(P.Props, 14, 26) AS OrderId, P.ChannelId FROM Posts P WHERE P.Props LIKE '{"order_id":"%"}' GROUP BY P.ChannelId) CWO ON C.Id = CWO.ChannelId
			LEFT JOIN Orders O ON O.Id = CWO.OrderId
			WHERE CM.UserId = :UserId AND C.Status = :Status AND ((C.Id = CWO.ChannelId AND O.DeliveryAt < :DeliveryAt) OR O.Id IS NULL OR C.Status = :Status)
			ORDER BY C.LastPostAt ASC`
		}
...
func (s SqlChannelStore) GetAllMembersForUser(userId string) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		var dbMembers channelMemberWithSchemeRolesList
		_, err := s.GetReplica().Select(&dbMembers, CHANNEL_MEMBERS_WITH_SCHEME_SELECT_QUERY+" WHERE ChannelMembers.UserId = :UserId", map[string]interface{}{"UserId": userId})

		if err != nil {
			result.Err = model.NewAppError("SqlChannelStore.GetAllMembersForUser", "store.sql_channel.get_members.app_error", nil, "userId="+userId+", err="+err.Error(), http.StatusInternalServerError)
			return
		}
	...
	return store.Do(func(result *store.StoreResult) {
		...
		if err := s.GetReplica().SelectOne(&channel, "SELECT Channels.* FROM Channels WHERE CreatorId = :CreatorId AND DeleteAt = 0 ORDER BY CreateAt", map[string]interface{}{"CreatorId": userId}); err != nil {
			if err == sql.ErrNoRows {
				...
				result.Err = model.NewAppError("SqlChannelStore.FindOpennedChannel", "store.sql_channel.get_by_name.existing.app_error", nil, "сreator_id="+userId+", "+err.Error(), http.StatusInternalServerError)
			}
		} else {
			result.Data = &channel
		}
	...
	return store.Do(func(result *store.StoreResult) {
		...
		if transaction, err := s.GetMaster().Begin(); err != nil {
			...
			if channelResult.Err != nil {
				...
				if member1Result.Err != nil {
					transaction.Rollback()
					result.Err = model.NewAppError("SqlChannelStore.CreateUnresolvedChannel", "store.sql_channel.create_unresolved_channel.add_members.app_error", nil, member1Result.Err.Error(), http.StatusInternalServerError)
					...
					if error {
						...
						if err := transaction.Commit(); err != nil {
							result.Err = model.NewAppError("SqlChannelStore.CreateUnresolvedChannel", "store.sql_channel.create_unresolved_channel.commit.app_error", nil, err.Error(), http.StatusInternalServerError)
						} else {
