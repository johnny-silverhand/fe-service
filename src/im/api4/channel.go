...
func (api *API) InitChannel() {
	...
	api.BaseRoutes.Channel.Handle("/patch", api.ApiHandler(patchChannel)).Methods("PUT")
	...
	api.BaseRoutes.ChannelAllMembersForUser.Handle("", api.ApiSessionRequired(getAllChannelMembersForUser)).Methods("GET")
...
func getAllChannelMembersForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	c.RequireUserId()
	...
	members, err := c.App.GetChannelAllMembersForUser(c.Params.UserId)
	if err != nil {
		c.Err = err
		return
	}
