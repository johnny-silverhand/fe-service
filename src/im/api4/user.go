...
func verifyUserPhoneNew(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	_, err := c.App.VerifyFromStageTokenPhoneNew(token, code)
...
func sendVerificationPhoneNewSms(c *Context, w http.ResponseWriter, r *http.Request) {
	props := model.MapFromJson(r.Body)
	token := props["token"]

	if err := c.App.SendVerifyFromStagePhoneNewToken(token); err != nil {
		c.Err = err
		return
	}
...
func verifyUserPhone(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	user, err := c.App.VerifyFromStageToken(token, code)
...
func sendVerificationSms(c *Context, w http.ResponseWriter, r *http.Request) {
	props := model.MapFromJson(r.Body)
	token := props["token"]

	if err := c.App.SendVerifyFromStageToken(token); err != nil {
		c.Err = err
		return
	}
