...
func (a *App) UpdatePost(post *model.Post, safeUpdate bool) (*model.Post, *model.AppError) {
	...
	*newPost = *oldPost
	...
	if newPost.EditAt == oldPost.EditAt && (!oldPost.FileIds.Equals(newPost.FileIds)) {
		newPost.EditAt = model.GetMillis()
	}
...
func (a *App) GetAllMessagesBeforeMessage(userId, postId string, page, perPage int) (*model.MessageArray, *model.AppError) {
	...
	if result := <-a.Srv.Store.Post().GetAllMessagesBefore(userId, postId, perPage, page*perPage, limitMin); result.Err != nil {
		...
		return result.Data.(*model.MessageArray), nil
	}
}

func (a *App) GetAllMessagesAfterMessage(userId, postId string, page, perPage int) (*model.MessageArray, *model.AppError) {
...
func (a *App) GetAllMessagesAroundMessage(postId, userId string, offset, limit int, before bool) (*model.MessageArray, *model.AppError) {
	var pchan store.StoreChannel
	var limitMin int64
	limitMin = 90 * 24 * 60 * 60000

	if before {
		pchan = a.Srv.Store.Post().GetAllMessagesBefore(userId, postId, limit, offset, limitMin)
	} else {
		pchan = a.Srv.Store.Post().GetAllMessagesAfter(userId, postId, limit, offset, limitMin)
	}
...
func (a *App) GetAllMessagesSince(userId string, time int64) (*model.MessageArray, *model.AppError) {
	var limitMin int64
	limitMin = 90 * 24 * 60 * 60000

	if result := <-a.Srv.Store.Post().GetAllMessagesSince(userId, time, true, limitMin); result.Err != nil {
		return nil, result.Err
	} else {
		return result.Data.(*model.MessageArray), nil
	}
...
func (a *App) GetAllMessagesPage(userId string, page int, perPage int) (*model.MessageArray, *model.AppError) {
	var limitMin int64
	limitMin = 90 * 24 * 60 * 60000

	if result := <-a.Srv.Store.Post().GetAllMessages(userId, page*perPage, perPage, true, limitMin); result.Err != nil {
		return nil, result.Err
	} else {
		return result.Data.(*model.MessageArray), nil
	}
