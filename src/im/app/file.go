...
const (
	...
	*/
	...
	ImageThumbnailWidth  = 240
	ImageThumbnailHeight = 240
	...
	// Deprecated
	IMAGE_THUMBNAIL_PIXEL_WIDTH  = 240
	IMAGE_THUMBNAIL_PIXEL_HEIGHT = 240
...
func (a *App) GetInfoForFilename(post *model.Post, /*teamId string*/ filename string) *model.FileInfo {
	...
	if split[0] != "" || split[1] != post.ChannelId || split[2] != post.UserId || strings.Contains(split[4], "/") {
		mlog.Warn(
			...
			mlog.String("metadata_id", post.Id),
	...
	pathPrefix := fmt.Sprintf("/channels/%s/users/%s/%s/",  channelId, userId, oldId)
	...
	info.MetadataId = post.Id
	info.MetadataType = model.METADATA_TYPE_POST
...
func (a *App) FindTeamIdForFilename(post *model.Post, filename string) string {
	...
	for _, team := range teams {
		path := fmt.Sprintf("/channels/%s/users/%s/%s/%s", /*team.Id, */post.ChannelId, post.UserId, id, name)
...
func (a *App) MigrateFilenamesToFileInfos(post *model.Post) []*model.FileInfo {
	...
	if teamId == "" {
		...
		for _, filename := range filenames {
			info := a.GetInfoForFilename(post, filename)
			if info == nil {
				continue
			}
...
func (a *App) UploadMultipartFiles( channelId string, userId string, fileHeaders []*multipart.FileHeader, clientIds []string, now time.Time) (*model.FileUploadResponse, *model.AppError) {
...
func (a *App) UploadFile(data []byte, channelId string, filename string) (*model.FileInfo, *model.AppError) {
	info, _, appError := a.DoUploadFileExpectModification(time.Now(),  channelId, "nouser", filename, data)
...
func (a *App) DoUploadFile(now time.Time, rawChannelId string,  rawUserId string, rawFilename string, data []byte) (*model.FileInfo, *model.AppError) {
	info, _, err := a.DoUploadFileExpectModification(now, rawChannelId, rawUserId, rawFilename, data)
	return info, err
}
...
func (a *App) DoUploadFileExpectModification(now time.Time, /*rawTeamId string, */rawChannelId string, rawUserId string, rawFilename string, data []byte) (*model.FileInfo, []byte, *model.AppError) {
	...
	pathPrefix := now.Format("20060102") +  "/users/" + userId + "/" + info.Id + "/"
...
func (a *App) CopyFileInfos(userId string, fileIds []string) ([]string, *model.AppError) {
	...
	for _, fileId := range fileIds {
		...
		fileInfo.MetadataId = ""
		fileInfo.MetadataType = model.METADATA_TYPE_POST
