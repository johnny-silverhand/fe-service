...
func (a *App) PreCreateUser(user *model.User) (*model.User, *model.AppError) {
	...
	if result := <-a.Srv.Store.User().SaveWithCheckExist(user); result.Err != nil {
...
func (a *App) CreateUserWithToken(user *model.User, pwd string) (*model.Token, *model.AppError) {
	...
	token := model.NewStageToken(ruser.Id, pwd)
...
func (a *App) VerifyFromStageTokenPhoneNew(userSuppliedTokenString string, code string) (*model.User, *model.AppError) {
	...
	if token, err = a.GetStageToken(userSuppliedTokenString); err != nil {
		...
		if model.GetMillis()-token.CreateAt >= TOKEN_RECOVER_EXPIRY_TIME {
			return nil, model.NewAppError("resetPassword", "api.user.verified.link_expired.app_error", nil, "", http.StatusBadRequest)
		}
		...
		if err := a.VerifyUserPhoneNew(token.UserId); err != nil {
			return nil, err
		}
...
func (a *App) GetStageToken(token string) (*model.Token, *model.AppError) {
	if result := <-a.Srv.Store.Token().GetByToken(token); result.Err != nil {
		...
		token := result.Data.(*model.Token)
		/*if token.Type != TOKEN_TYPE_PASSWORD_RECOVERY {
			return nil, model.NewAppError("GetPasswordRecoveryToken", "api.user.get_stage_token.broken_token.app_error", nil, "", http.StatusBadRequest)
		}*/
...
func (a *App) VerifyUserPhoneNew(userId string) *model.AppError {
	return (<-a.Srv.Store.User().VerifyPhoneNew(userId)).Err
}
...
func (a *App) UpdateExtraStageToken(token *model.Token, newExtra string) *model.AppError {

	if result := <-a.Srv.Store.Token().UpdateExtra(token.Token, newExtra); result.Err != nil {
		return model.NewAppError("UpdateExtra", "api.user.update_password.failed.app_error", nil, result.Err.Error(), http.StatusInternalServerError)
	}

	return nil
}

func (a *App) SendVerifyFromStagePhoneNewToken(userSuppliedTokenString string) *model.AppError {
	var token *model.Token
	var err *model.AppError
	if token, err = a.GetStageToken(userSuppliedTokenString); err != nil {
		return err
...
func (a *App) GetUserByPhone(phone string) (*model.User, *model.AppError) {
	...
	if result := <-a.Srv.Store.User().GetByPhone(phone); result.Err != nil && result.Err.Id == "store.sql_user.missing_account.const" {
		result.Err.StatusCode = http.StatusNotFound
		return nil, result.Err
...
func (a *App) SendVerifyFromStageTokenPush(userSuppliedTokenString string) *model.AppError {
	...
	a.Srv.Go(func() {
		...
		if user.NotifyProps[model.PUSH_NOTIFY_PROP] == model.USER_NOTIFY_ALL && channel != nil {
			a.SendCustomNotifications(user, channel, "Код подтверждения: "+token.Extra, "confirm")
		}
	})
